...
/*
Optimiza el refresco del resumen con las relaciones validadas en produccion:
1. caso_calificado no contiene cedula; la sala se resuelve por radicado.
2. las notificaciones activas se relacionan directamente por id_caso.
3. se eliminan el OR y el EXISTS correlacionado que multiplicaban las lecturas.
*/

IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_calificacion_sistema_caso_resumen_radicado'
      AND object_id = OBJECT_ID('jnc.calificacion_sistema_caso')
)
BEGIN
    CREATE INDEX IX_calificacion_sistema_caso_resumen_radicado
    ON jnc.calificacion_sistema_caso (
        numero_radicado_normalizado,
        fecha_audiencia DESC,
        id_calificacion_sistema_caso DESC
    )
    INCLUDE (sala)
    WHERE activo = 1;
END;
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_notificacion_esperada_resumen_id_caso'
      AND object_id = OBJECT_ID('jnc.notificacion_esperada')
)
BEGIN
    CREATE INDEX IX_notificacion_esperada_resumen_id_caso
    ON jnc.notificacion_esperada (id_caso)
    INCLUDE (
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_audiencia_caso_resumen_radicado'
...
BEGIN
    ...
    notificacion_estado AS (
        SELECT
            ne.id_caso,
            UPPER(COALESCE(ne.tipo_destinatario, 'SIN_TIPO_DESTINATARIO'))
                AS tipo_destinatario,
            COALESCE(
    ...
    resumen_por_radicado AS (
        ...
        GROUP BY numero_radicado_normalizado
    )
